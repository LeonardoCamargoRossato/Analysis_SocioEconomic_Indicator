...
def display_binscatters(countries):
 ...
 #################################################################################################################
    ...
    def binscatter_functions_main(df_selected, specific_color):
        st.markdown("<hr style='border: 0.01 px solid white;'>", unsafe_allow_html=True)

        st.markdown("### Binscatter: Distribution Analysis and Histograms")
        col1, col2 = st.columns(2)
        with col1:
            n = st.slider("Select Number of Bins (for distribution division: Nº bins = 10^n)", 
                        min_value=1, max_value=100, value=1, step=1)

        with col2:
            bin_separation_mode = st.selectbox("Select Bin Mode Separation", 
                                            options=['exponential', 'linear'])
        create_and_plot_bins_graphs(n, df_selected, bin_separation_mode, axis_x, axis_y)

        st.markdown("Binscatter: Fit Curve per Binscatter")
        height = 500; width = 1000
        binscatter_and_fit_curve(n, df_selected, bin_separation_mode, axis_x, axis_y, height, width)

        st.markdown("# Binscatter vs Scatter")    
        opacity = st.slider("Select Opacity (Cities Points - background)", min_value=0.01, max_value=1.0, value=0.5, step=0.01)
        create_and_plot_bins_graphs__scatter_x_binscatter(n, df_selected, bin_separation_mode, axis_x, axis_y, height, width, opacity)
    ...
    if st.session_state.region_mode == "Per Country":
        ...
        binscatter_functions_main(df_selected, specific_color)
        ...
        df_selected = df[df[macrorregion[1]] == str(select_macrorregion)]
        ...
        df_selected = df[df[state[1]] == str(select_state)]
